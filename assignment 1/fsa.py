...
class FSA(): # Finite State Automata
    ...
    def is_all_transitions_are_in_alpha(self): # E3
        for tr in self.trans:
            fr, op, to = tr.split(">")
    ...
    def is_complete(self): # Report
        for fr in self.graph.keys():
            opertions_from = [op for op, to in self.graph[fr]]
            if set(opertions_from) != set(self.alpha):
                return False
        return True    
    
        
    def is_accepting_site_defined(self): # W1
        return bool(self.fin_st)
    
    
    def are_all_states_reachable(self): # W2
        print(self.graph)
        self.used = dict([(s, False) for s in self.states])
        self.dfs(self.init_st)
        all_visited = dict([(s, True) for s in self.states])
        return self.used == all_visited        
    
    
    def is_deterministic(self): # W3
        for fr in self.graph.keys():
...
def solution(input_data):
    error = completeness = warnings = None
    
    if not is_input_file_correct(input_data):
        error = "E5: Input file is malformed"
        return (error, completeness, warnings)
    
    fsa = FSA(proceed_data(input_data))
    q = fsa.is_all_transitions_are_in_alpha()
    if not fsa.is_s_in_set_of_states():
        error = "E1: A state '{}' is not in set of states".format(fsa.init_st)
    elif not fsa.are_all_states_joint():
        error = "E2: Some states are disjoint"
    elif not q[0]:
        error = "E3: A transition '{}' is not represented in the alphabet".format(q[1])
    elif not fsa.is_init_state_is_defined():
        error = "E4: Initial state is not defined"
        
    completeness = "FSA is " + ["in", ""][fsa.is_complete()] + "complete"
    warnings = list()
    if not fsa.is_accepting_site_defined():
        warnings.append("W1: Accepting state is not defined")
    if not fsa.are_all_states_reachable():
        warnings.append("W2: Some states are not reachable from initial state")
...
def main():
    ...
    error, completeness, warnings = solution(input_data)
